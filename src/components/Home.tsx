...
import { Sparkles, FileText, Camera, Search, ArrowRight, Clock, Target, Globe, Zap } from 'lucide-react';
import { Difficulty } from '../types';
import Scanner from './Scanner';

interface HomeProps {
  onGenerate: (text: string, difficulty: Difficulty, numQuestions: number, language: string) => void;
  onScan: (text: string, difficulty: Difficulty, numQuestions: number, language: string, topic: string) => void;
  onCurrentAffairs: (numQuestions: number, language: string, difficulty: Difficulty) => void;
  loading: boolean;
}
...
const Home: React.FC<HomeProps> = ({ onGenerate, onScan, onCurrentAffairs, loading }) => {
  ...
  const handleSample = (sampleTopic: string) => {
    setTopic(sampleTopic);
    if (sampleTopic === "Biology") {
  ...
  const handleGenerate = () => {
    ...
    onGenerate(fullText, difficulty, numQuestions, language);
  };

  const handleScanCapture = (text: string) => {
    setIsScannerOpen(false);
    onScan(text, difficulty, numQuestions, language, topic);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 md:py-20 px-4 md:px-6">
            ...
            <div className="flex flex-col gap-4 w-full md:w-auto">
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-3">
                {[5, 10, 20].map((num) => (
                  <button
                    key={num}
                    onClick={() => onCurrentAffairs(num, language, difficulty)}
                    disabled={loading}
      ...
      <div className="relative">
        <div className="absolute -top-20 -left-20 w-64 h-64 bg-blue-400/10 rounded-full blur-3xl -z-10 animate-float" />
        <div className="absolute -bottom-20 -right-20 w-64 h-64 bg-indigo-400/10 rounded-full blur-3xl -z-10" style={{ animationDelay: '3s' }} />
        ...
        >
          ...
          <div className="flex border-b border-slate-100 bg-slate-50/50">
            <button 
              ...
              className={`flex-1 py-6 font-black text-xs uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3 ${inputMode === "paste" ? 'text-blue-600 bg-white border-b-4 border-blue-600' : 'text-slate-400 hover:text-slate-600'}`}
            >
              <FileText className="w-4 h-4" />
              Paste Notes
            </button>
            <button 
              onClick={() => { setInputMode("scan"); setIsScannerOpen(true); }}
              className={`flex-1 py-6 font-black text-xs uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-3 ${inputMode === "scan" ? 'text-indigo-600 bg-white border-b-4 border-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}
            >
          ...
          <div className="p-6 md:p-12">
              ...
              <div className="relative group">
                {inputMode === "scan" && notes.length === 0 ? (
                  <div className="w-full h-72 md:h-96 flex flex-col items-center justify-center bg-slate-50/50 border-2 border-dashed border-slate-200 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 text-center">
                    ...
                    <p className="text-slate-500 font-medium mb-6 md:mb-8 max-w-xs leading-relaxed text-sm md:text-base">
                      Point your camera at your notes and we'll handle the rest.
                    </p>
                    <button 
                  ...
                  <>
                    <textarea 
                      ...
                      placeholder={inputMode === "paste" ? "Paste your notes, article text, or study material here..." : "Captured text will appear here..."}
                      value={notes}
                      onChange={(e) => setNotes(e.target.value)}
                      maxLength={MAX_CHARS}
                      readOnly={inputMode === "scan"}
                    />
            ...
            <button 
              ...
              className={`w-full py-5 md:py-6 rounded-[1.5rem] md:rounded-[2rem] font-black text-xl md:text-2xl shadow-2xl transition-all flex items-center justify-center gap-4 relative overflow-hidden group ${isButtonDisabled ? 'bg-slate-100 text-slate-400 cursor-not-allowed border-2 border-slate-200' : 'bg-gradient-to-r from-blue-600 via-indigo-700 to-violet-700 text-white transform active:scale-[0.98] hover:shadow-blue-500/30'}`}
            >
              {loading ? (
                <>
      ...
      <div className="mt-24 md:mt-32 grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
        {[
          ...
          >
            <div className={`w-16 h-16 ${feature.color} rounded-2xl flex items-center justify-center mb-8 group-hover:scale-110 transition-transform duration-500 group-hover:rotate-3`}>
              {feature.icon}
            </div>
      ...
      <div className="mt-20 md:mt-32 space-y-24 md:space-y-40">
        ...
        <section id="learning-science" className="bg-slate-900 -mx-4 md:-mx-12 px-6 md:px-20 py-20 md:py-32 rounded-[2.5rem] md:rounded-[4rem] text-white relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 md:w-96 h-64 md:h-96 bg-blue-500/10 rounded-full blur-[80px] md:blur-[120px]" />
          <div className="absolute bottom-0 left-0 w-64 md:w-96 h-64 md:h-96 bg-indigo-500/10 rounded-full blur-[80px] md:blur-[120px]" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 items-start">
              <div className="space-y-8 text-slate-400 font-medium text-base md:text-lg leading-relaxed">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-4">
                    <div className="flex-shrink-0 w-1.5 h-1.5 mt-2.5 bg-indigo-500 rounded-full" />
                  ...
                  <div className="flex gap-4">
                    <div className="flex-shrink-0 w-1.5 h-1.5 mt-2.5 bg-violet-500 rounded-full" />
        ...
        <section id="categories" className="max-w-5xl mx-auto">
          <div className="text-center mb-16 md:mb-24">
            ...
            <h2 className="text-3xl md:text-6xl font-black text-slate-900 tracking-tight font-display mb-8">QuickQuiz Photo to Quiz Tool</h2>
              ...
              >
                ...
                <ul className="space-y-3">
                  {cat.items.map((item, j) => (
                    <li key={j} className="flex items-center gap-3 font-bold text-sm md:text-base opacity-80">
        ...
        <section id="how-it-works" className="max-w-5xl mx-auto pb-20 md:pb-32">
          <div className="text-center mb-16 md:mb-24">
            ...
            <h2 className="text-3xl md:text-6xl font-black text-slate-900 tracking-tight font-display mb-8">QuickQuiz Online Quiz Maker for Students</h2>
          </div>
              ...
              <div key={i} className="flex flex-col lg:flex-row gap-10 md:gap-20 items-center">
                <div className="lg:w-1/2 space-y-6 md:space-y-8">
                  <div className="flex items-center gap-6">
                    <span className="text-5xl md:text-7xl font-black text-slate-100 font-display">{step.step}</span>
                    <div className={`w-16 h-16 md:w-20 md:h-20 ${step.color} rounded-3xl flex items-center justify-center text-white shadow-2xl shadow-current/20`}>
                      {step.icon}
                    </div>
                  </div>
                  <h3 className="text-2xl md:text-4xl font-black text-slate-900 font-display">{step.title}</h3>
                  <p className="text-slate-600 text-lg md:text-xl font-medium leading-relaxed">{step.desc}</p>
                  <p className="text-slate-400 text-sm md:text-base font-medium leading-relaxed italic border-l-4 border-slate-100 pl-6">{step.details}</p>
                </div>
                <div className="lg:w-1/2 w-full">
                  <div className="aspect-video bg-slate-50 rounded-[2.5rem] border-2 border-slate-100 overflow-hidden relative group">
                    <img src={`https://picsum.photos/seed/step${i}/800/600`} className="w-full h-full object-cover opacity-80 group-hover:scale-110 transition-transform duration-700" alt={step.title} referrerPolicy="no-referrer" />
                    <div className="absolute inset-0 bg-gradient-to-t from-slate-900/60 to-transparent" />
                    <div className="absolute bottom-8 left-8 right-8">
                      <div className="px-4 py-2 bg-white/10 backdrop-blur-md border border-white/20 rounded-xl inline-block text-[10px] font-black text-white uppercase tracking-widest mb-2">Step {step.step} Preview</div>
                      <p className="text-white/80 text-xs font-bold">Visualizing the {step.title.toLowerCase()} process.</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <Scanner isOpen={isScannerOpen} onClose={() => setIsScannerOpen(false)} onCapture={handleScanCapture} />
    </div>
  );
};
