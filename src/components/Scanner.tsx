...
import { Camera, X, RefreshCcw, ArrowRight, Sparkles, CircleAlert, LoaderCircle, ChevronLeft, Zap } from 'lucide-react';
  ...
  const processImage = async () => {
    if (capturedImage) {
      ...
      try {
        ...
        // 2. Refine the text using AI to fix OCR errors
        setStatus("Refining text with AI...");
        const { cleanText } = await import('../services/aiService');
        const refinedText = await cleanText(rawText);
        
        onCapture(refinedText);
      } catch (err: any) {
        ...
        <div className="relative flex-grow bg-slate-900 overflow-hidden flex items-center justify-center min-h-[300px] md:min-h-[400px]">
          <AnimatePresence>
            {isInitializing && (
              ...
              >
                <LoaderCircle className="w-10 h-10 md:w-12 md:h-12 text-blue-500 animate-spin mb-4 md:mb-6" />
            ...
            {isProcessing && (
              ...
              >
                  ...
                  <div className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-white/10 flex items-center justify-center">
                    ...
                    <svg className="absolute inset-0 w-full h-full -rotate-90">
                      ...
                      <circle 
                        ...
                        style={{ strokeDasharray: "289", strokeDashoffset: 289 - (289 * progress) }}
                      />
                    </svg>
                  </div>
                ...
                <p className="text-slate-400 text-xs md:text-sm font-bold uppercase tracking-[0.2em] mb-8 text-center">{status || "Extracting text from image..."}</p>
          ...
          {error && !isProcessing ? (
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center p-8 md:p-12 text-center bg-slate-900">
              <div className="bg-rose-500/20 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] mb-6 md:mb-8">
                <CircleAlert className="w-12 h-12 md:w-16 md:h-16 text-rose-500" />
              </div>
            ...
            <div className="w-full h-full relative">
              ...
              <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
                <div className="w-[85%] h-[80%] border-2 border-dashed border-white/30 rounded-[2rem] md:rounded-[3rem] relative">
                  ...
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white/20 text-[8px] md:text-[10px] font-black uppercase tracking-[0.4em] text-center w-full">Align text within frame</div>
                </div>
              </div>
              {/* Flash Effect */}
              <AnimatePresence>
